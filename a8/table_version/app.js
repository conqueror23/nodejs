...
io.sockets.on('connection',function (socket){
  ...
  socket.on('search',function(word){
    MongoClient.connect(url,function(err,client){
      ...
      console.log(word);
      const db  = client.db(dbName);
      db.collection("product").find({company:new RegExp(word, 'i')}).toArray(function(err,p_data){
         if(err) throw err;
        console.log(p_data);

        socket.emit('searchResult',p_data)
      })
      client.close();
    })
  ...
  socket.on('commentl',function (data){
    MongoClient.connect(url,function (err,client){
      ...
      db.collection("comments").find({product:data}).toArray(function(err,cd){
        if(err) throw err;
        console.log("cd shows here "+cd[0].comments);
        console.log(cd[0].comments);
